...
function saveEditSong(id, info) {
    ...
    if(info.others != null) {
        update["songOthers"] = String(info.others);
    }
    if(info.sheet != null) {
        processSheetMusic(info, id);
    }
...
function saveNewSong(info, id) {
    
    var sheet = info.sheet;
    var collect = sheet.match(/[\"].+[\"]/gi);
    ...
    var newSong = new Songs({
        ...
        songOthers: String(info.others),
        ownerId: "testUser",
        songLyrics: lyrics,
        songSheet: sheet,
        songChords: chords,
        songSections: collect,
        songHeaders: headers,
        isActive: 1
    })
...
function deleteItem(id) {
    var update = getObject(id).objectId;
        ...
        update["isActive"] = 0;
        update = Backendless.Persistence.of(Songs).save(update);
    console.log("Song deleted: " + update);
}
...
function deleteSetlist(id) {
    ...
    console.log("Setlist deleted " + update);
}
...
function saveEditedSetlist(info, objectId) {
    var update = Backendless.Persistence.of(Setlists).findById(objectId);
        update["setlistName"] = info.name;
        update["setlistSpotify"] = info.spotify;
        update["setlistNotes"] = info.notes;
